```vue
<template>
    <v-card class="d-flex flex-column justify-space-between w-100 pa-6 rounded-2xl elevation-2 position-relative h-100 white-news-card">
        <div class="position-relative overflow-hidden rounded-lg mb-5">
            <v-img
                :src=" item.image"
                height="180"
                cover
                alt="تصویر خبر"
                class="rounded-lg"
            >
                <template v-slot:placeholder>
                    <v-row class="fill-height ma-0" align="center" justify="center">
                        <v-progress-circular
                            indeterminate
                            color="primary"
                        ></v-progress-circular>
                    </v-row>
                </template>
            </v-img>

            <span class="position-absolute rounded-pill font-weight-bold text-body-2 white--text px-3 py-1 category-tag">
                {{ item.category || 'اطلاعیه' }}
            </span>
        </div>

        <div class="d-flex flex-column flex-grow-1">
            <div class="d-flex align-center mb-4 text-body-2 font-weight-medium blue-grey--text text--darken-2">
                <div class="d-flex align-center ml-4">
                    <v-icon small color="#64748b" class="ml-1">
                        mdi-calendar-month-outline
                    </v-icon>
                    <span>{{ item.date || '۱۵ شهریور ۱۴۰۳' }}</span>
                </div>

                <div class="d-flex align-center">
                    <v-icon small color="#64748b" class="ml-1">
                        mdi-clock-outline
                    </v-icon>
                    <span>{{ item.readTime || '۲ دقیقه مطالعه' }}</span>
                </div>
            </div>

            <h3 class="font-weight-bold text-subtitle-1 mb-3 overflow-hidden blue-grey--text text--darken-4 news-title">
                {{ item.title }}
            </h3>

            <p class="font-weight-regular text-body-1 mb-6 overflow-hidden blue-grey--text text--darken-3 news-excerpt">
                {{ item.excerpt || item.description }}
            </p>
        </div>

        <div class="mt-auto">
            <v-divider class="mb-4"></v-divider>
            <div class="d-flex align-center justify-space-between">
                <div class="d-flex align-center text-caption font-weight-bold blue-grey--text text--darken-2">
                    <v-icon small color="#64748b" class="ml-1">
                        mdi-account-circle-outline
                    </v-icon>

                    <span>توسط: پشتیبانی فنی</span>
                </div>

                <BaseButton
                    :to="`/news/${item.id}`"
                    color="#0f172a"
                    elevation="0"
                    :block="false"
                    :x-large="false"
                    c-class="white--text font-weight-bold px-4 rounded-lg"
                >
                    ادامه مطلب
                    <v-icon small class="mr-1">
                        mdi-arrow-left
                    </v-icon>
                </BaseButton>
            </div>
        </div>
    </v-card>
</template>

<script>
import BaseButton from '~/components/Base/BaseButton.vue'

export default {
    name: 'NewsCard',

    components: {
        BaseButton
    },

    props: {
        item: {
            type: Object,
            required: true
        }
    }
}
</script>

<style scoped>
.news-title {
    line-height: 1.5;
    display: -webkit-box;
    -webkit-box-orient: vertical;
    height: 3em;
}

.news-excerpt {
    line-height: 1.6;
    display: -webkit-box;
    -webkit-box-orient: vertical;
    height: 4.8em;
}

.category-tag {
    top: 16px;
    right: 16px;
    background-color: #0f172a;
    z-index: 2;
    line-height: 1.2;
}
</style>
